...
def to_filename(directory, url):
    ...
    """
    print ("dir: "+directory+" url: "+url)
    if url.startswith("file:"):
        print ("echo:"+os.path.join(directory, url.split("e:/")[1].replace("/", "_")))
        return os.path.join(directory, url.split("e:/")[1].replace("/", "_"))
...
def format_number(number, SI=0, space=' '):
    ...
    if type(number) == type(1) or isinstance(number, long):
